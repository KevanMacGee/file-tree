...
def build_tree_string(dir_path, prefix="", spec=None, root_dir=None, show_all=False):
    ...
    try:
        ...
        for i in items:
            if i.startswith('$'):
                continue
            full = os.path.join(dir_path, i)
            if not show_all and is_os_hidden(full):
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("-a", "--all", action="store_true", help="Show all files, including OS-hidden files and .gitignore-excluded entries")
    ...
    gitignore_spec = None if (args.all or args.no_ignore) else get_gitignore_spec(args.path)
